...
const UPDATE_HISTORY = [
  {
    ...
    title: "CPUを調整",
    items: [
      "超強いCPUの行動順と召喚判断を調整しました。"
    ]
  },
  {
    version: "v1.27",
    title: "超強いCPUの無駄行動を調整",
    items: [
      "超強いCPUがレッドカードを使う条件を、相手の手札が4枚以上ある時にしました。",
      "超強いCPUが、倒せない相手モンスターへ中途半端に攻撃しにくいようにしました。",
      "全体攻撃などで他のモンスターを倒せる場合は、攻撃候補として残るようにしています。"
    ]
  },
  {
    version: "v1.26",
    title: "超強いCPUを追加",
    items: [
      "CPU戦に「超強い」を追加しました。強いCPUの判断をベースに、より勝ち筋と盤面価値を重く見る難易度です。",
      "超強いCPUは、コートチェンジ、二重チェック、発掘、ザ・サーチなどの重要カードをより高く評価します。",
      "願い事は相手モンスターを倒せる見込みがある時に使いやすくし、バトルドラムは攻撃できるモンスターが複数いる場面を重視するようにしました。",
      "ミミッキュ、サンダー、ガオガエン、カバルドン、ピカチュウなどの攻撃代わりの効果を、無駄撃ちしにくいよう調整しました。",
      "持ち物の装備先評価を調整し、ライフパワー、拘りスカーフ、ブーメラン、気合いのタスキなどをより相性のよいモンスターに持たせやすくしました。"
    ]
  },
  {
    version: "v1.25",
    title: "CPU（強い）の判断を調整",
    items: [
      "ライフ攻撃や石投げなどで勝てる場面では、勝ちに向かう行動を優先するようにしました。",
      "ヌオーがいる時に無意味なパワー上昇を使いにくくし、ヌオー自身にも火力アップ系の持ち物を持たせにくくしました。",
      "ライフパワーは基本的にパワーが下がる相手には使わず、高HP低パワーのモンスターを優先するようにしました。",
      "未公開の相手持ち物をCPUが中身まで知っているような評価をしにくくしました。"
    ]
  },
  {
    version: "v1.24",
...
const elements = {
  ...
  cpuUltraButton: document.querySelector("#cpuUltraButton"),
...
function restoreCpuBattleIfNeeded() {
  ...
  cpuDifficulty = ["hard", "ultra"].includes(saved.cpuDifficulty) ? saved.cpuDifficulty : "normal";
  cpuEnabled = true;
  cpuThinking = false;
  onlineMode = false;
  onlineState = null;
...
function cpuProfile(difficulty) {
  ...
  const name = difficulty === "ultra" ? "CPU\uFF08\u8D85\u5F37\u3044\uFF09" : difficulty === "hard" ? "CPU\uFF08\u5F37\u3044\uFF09" : "CPU\uFF08\u666E\u901A\uFF09";
  return { name, avatar };
}
...
function isHardLikeCpu() {
  return cpuDifficulty === "hard" || cpuDifficulty === "ultra";
}

function isUltraCpu() {
  return cpuDifficulty === "ultra";
}
...
elements.cpuUltraButton?.addEventListener("click", () => startCpuGame("ultra"));
...
async function runHardCpuTurn() {
  ...
  let lethalAction = chooseCpuLethalActionChoice();
  if (lethalAction) {
    logCpuDecision("lethal-action", Math.round(lethalAction.score), { phase: "pre-equip" });
    await executeHardCpuChoice(lethalAction);
    await runCpuPendingDiscardTake();
    if (game.winner === null && game.activePlayer === 1) {
  ...
  lethalAction = chooseCpuLethalActionChoice();
  if (lethalAction) {
    logCpuDecision("lethal-action", Math.round(lethalAction.score), { phase: "post-equip" });
    await executeHardCpuChoice(lethalAction);
    await runCpuPendingDiscardTake();
    if (game.winner === null && game.activePlayer === 1) {
      await cpuStep("CPU ターン終了", () => engine.endTurn(game, 1), "turn");
  ...
  if (isUltraCpu()) {
    await runUltraCpuEarlySpecialAbilities();
    await runCpuPendingDiscardTake();
  }
  ...
  while (game.winner === null && game.activePlayer === 1 && game.players[1].actions > 0 && guard < 8) {
    ...
    lethalAction = chooseCpuLethalActionChoice();
    if (lethalAction) {
      logCpuDecision("lethal-action", Math.round(lethalAction.score), { phase: "main-followup" });
      await executeHardCpuChoice(lethalAction);
      break;
    }
  ...
  if (!canCpuLethalByAttacksNow() && !chooseCpuLethalActionChoice()) await runHardCpuUnitAbilities();
  await runCpuPendingDiscardTake();
  await runCpuEquipItems();
  await runHardCpuAttacks();
  await runCpuPendingDiscardTake();
...
async function runUltraCpuEarlySpecialAbilities() {
  if (!isUltraCpu() || game.winner !== null || game.activePlayer !== 1) return false;
  let acted = false;
  let guard = 0;
  while (game.winner === null && game.activePlayer === 1 && guard < 3) {
    guard += 1;
    if (canCpuLethalByAttacksNow() || chooseCpuLethalActionChoice() || isCpuInLethalDanger()) break;
    const unit = game.players[1].field.find((candidate) => {
      const card = CARD_DEFINITIONS[candidate.cardId];
      return candidate.canAct
        && card?.effectKey === "gainSpecialInsteadAttack"
        && cpuKnownItemCardId(candidate, 1) !== "lightBall";
    });
    if (!unit) break;
    const result = await cpuStep("CPU スペシャル入手", () => {
      addFx(`field:1:${unit.id}`, "fx-item");
      return engine.useUnitAbility(game, 1, { ability: "gainSpecial", unitId: unit.id });
    }, "select");
    if (!result?.ok) break;
    acted = true;
  }
...
async function runCpuEquipItems() {
  ...
  while (equipped) {
    equipped = false;
    const choice = isHardLikeCpu() ? chooseHardCpuItemEquip() : chooseCpuItemEquip();
    if (!choice) return;
...
function scoreItemOnUnit(itemId, unit) {
  ...
  if (hasCpuOwnQuagsireActive() && isPowerIncreaseItem(itemId)) return -1000;
  if (itemId === "lightBall") return unit.cardId === "pikachu" ? 900 : -1000;
  if (card.effectKey === "ignorePowerIncreases" && ["lightBall", "choiceBand", "lifePower", "contraryMask"].includes(itemId)) return -1000;
  ...
  if (itemId === "choiceScarf") {
    if (unit.canAct && !isUltraCpu()) return 40;
    const aceBonus = ["kyogre", "calyrexShadow", "calyrexIce", "zacian", "urshifu", "calyrexIce"].includes(unit.cardId) ? 260 : 0;
    const reAttackBonus = unit.canAct ? 120 : 420;
    return reAttackBonus + aceBonus + hardUnitCardScore(card) + immediateAttackValue(unit) * (isUltraCpu() ? 0.75 : 0.45);
  }
  if (itemId === "lifePower") {
    const currentDamage = estimateLifeDamageForCpu(cpuEffectivePower(unit, null, "lifeAttack", 1), 0);
    const lifePowerDamage = estimateLifeDamageForCpu(Math.max(unit.hp, unit.power), 0);
    const enablesLethal = canCpuAttackLifeNow(unit) && lifePowerDamage >= game.players[0].life && currentDamage < game.players[0].life;
    if (unit.hp <= unit.power && !enablesLethal) return -1000;
    if (unit.hp < 4 && !enablesLethal) return -260;
  ...
  if (itemId === "focusSash") return (unit.maxHp <= 2 ? 340 : 180)
    + (unit.power >= 3 ? (isUltraCpu() ? 260 : 120) : 0)
    + (card.effectKey === "attackPowerPlusThree" ? 130 : 0)
    + (isUltraCpu() && unit.canAct && willLikelyDieNextTurn(unit) ? 260 : 0);
  if (itemId === "destinyCloak") return (unit.hp <= 2 ? 290 : 170)
    + (card.effectKey === "mustBeAttacked" ? 240 : 0)
    + (unit.power <= 1 ? 50 : 0)
    + (isUltraCpu() && unit.canAct && !willLikelyDieNextTurn(unit) ? 120 : 0);
  if (itemId === "boomerang") return unit.power >= (isUltraCpu() ? 3 : 2)
    ? 300 + game.players[0].field.length * (isUltraCpu() ? 150 : 90) + (unit.canAct ? 180 : 0)
    : 50;
...
function chooseHardCpuMainAction() {
  const choices = [
    ...cpuLethalActionChoices(),
    ...hardActionChoices(),
    ...hardSummonChoices(),
  ].map((choice) => ({ ...choice, score: choice.score + hardChoiceLookaheadScore(choice) }))
    .sort((a, b) => b.score - a.score);
  if (isUltraCpu()) {
    const setupChoices = choices.filter((choice) => choice.phase === "setup" && choice.score >= 120);
    if (setupChoices.length > 0) return chooseWeightedCpuChoice(setupChoices);
    const boardBuffChoices = choices.filter((choice) => choice.phase === "boardBuff" && choice.score >= 120);
    const summonChoices = choices.filter((choice) => choice.type === "summon" && choice.score >= 120);
    if (boardBuffChoices.length > 0 && summonChoices.length > 0) return chooseWeightedCpuChoice(summonChoices);
    const actionChoices = choices.filter((choice) => choice.type !== "summon" && choice.score >= 90);
    if (actionChoices.length > 0) return chooseWeightedCpuChoice(actionChoices);
  }
...
async function resolveCpuPendingChoices() {
  if (game.pendingOpponentHandCheck?.playerId === 1) {
    const indexes = game.players[0].hand
      .map((cardId, index) => ({ index, score: hardCardIdScore(cardId) + (isUltraCpu() && cardId === "courtChange" ? 1500 : 0) }))
      .sort((a, b) => b.score - a.score)
...
function hardSummonChoices() {
  ...
  return player.hand
    ...
    .filter((entry) => !(isUltraCpu() && anyTyranitarInField && entry.card.hp <= 1))
    .filter((entry) => !(isUltraCpu() && entry.cardId === "tyranitar" && ownHpOneInField))
    .map((entry) => {
      let score = 120 + hardUnitCardScore(entry.card) + ultraCardBonus(entry.cardId);
      if (player.field.length === 0) score += 220;
      if (player.field.length <= 2) score += 210;
      if (isUltraCpu() && player.field.length <= 2) score += 160;
...
function cpuLethalActionChoices() {
  return hardActionChoices().filter((choice) => choice.reason === "lethal" && choice.score >= 9000);
}

function chooseCpuLethalActionChoice() {
  return chooseWeightedCpuChoice(cpuLethalActionChoices().sort((a, b) => b.score - a.score));
}
...
function cpuReadyAttackers() {
  return game.players[1].field.filter((unit) => unit.canAct);
}

function canCpuKillTargetWithReadyAttackers(target) {
  if (!target) return false;
  const hp = effectiveDefenderHpForAttack(null, target);
  const total = cpuReadyAttackers()
    .filter((unit) => filterAttackTargets(game.players[0].field, unit).some((candidate) => candidate.id === target.id))
    .reduce((sum, unit) => sum + estimateUnitDamageForCpu(cpuEffectivePower(unit, target, "attack", 1), 0), 0);
  return total >= hp;
}

function bestCpuReadyKillValue() {
  return game.players[0].field.reduce((best, target) => {
    if (!canCpuKillTargetWithReadyAttackers(target)) return best;
    return Math.max(best, unitThreat(target) + priorityTargetBonus(target));
  }, 0);
}

function canCpuUseWishProfitably() {
  if (game.players[1].field.some((unit) => unit.canAct && filterAttackTargets(game.players[0].field, unit)
    .some((target) => effectiveDefenderHpForAttack(unit, target) <= estimateUnitDamageForCpu(cpuEffectivePower(unit, target, "attack", 1), 0)))) {
    return true;
  }
  return bestCpuReadyKillValue() >= 520;
}

function isUltraPowerBuffWasteful() {
  return isUltraCpu() && hasCpuOwnQuagsireActive();
}
...
function hardActionCandidates(handIndex, cardId) {
  ...
  const card = CARD_DEFINITIONS[cardId];
  const choices = [];
  const add = (score, payload = {}, reason = "value", phase = "tactical") => choices.push({ type: "action", handIndex, payload, score, reason, phase });
  ...
  const bestSearchPile = chooseBestCpuSearchPile();
  const enemyProtected = opponent.mysticGuardUntilTurn > game.turn;
  const ownQuagsireActive = hasCpuOwnQuagsireActive();
  ...
  switch (card.effectKey) {
    case "dealTwoToUnitOrLife":
      ...
      if (opponent.life <= estimateLifeDamageForCpu(3, 0, "action")) add(52000, { targetType: "life" }, "lethal");
      enemyTargets.forEach((unit) => {
        const kill = effectiveDefenderHpForAttack(null, unit) <= estimateUnitDamageForCpu(3, 0, "action");
    ...
    case "shockWave": {
      if (enemyProtected) break;
      const shockDamage = estimateUnitDamageForCpu(1, 0, "action");
      const kills = opponent.field.filter((unit) => effectiveDefenderHpForAttack(null, unit) <= shockDamage).length;
    ...
    case "specialCharge": {
      if (enemyProtected) break;
      player.field.forEach((unit) => {
        const rawDamage = cpuEffectivePower(unit, null, "status", 1);
        const lifeDamage = estimateLifeDamageForCpu(rawDamage, 0, "action");
        const kills = opponent.field.filter((target) => effectiveDefenderHpForAttack(unit, target) <= estimateUnitDamageForCpu(rawDamage, 0, "action")).length;
        let score = 120 + lifeDamage * 90 + kills * 520 - unitThreat(unit) * 0.35;
        if (lifeDamage >= opponent.life) score += 52000;
        if (kills > 0 && unit.hp <= 2) score += 180;
        if (score > 220) add(score, { unitId: unit.id }, lifeDamage >= opponent.life ? "lethal" : "remove");
      });
      break;
    }
    case "redCard":
    ...
    case "noCounterThisTurn":
      ...
      break;
    case "wish":
      if (canCpuUseWishProfitably()) {
        add((isUltraCpu() ? 680 : 300) + bestCpuReadyKillValue() * 0.35, {}, "value");
      }
      break;
    ...
    case "drawTwoGainAction":
      if (bestPile && drawRoomAfterPlaying() >= 2) add(280 + pileTopScore(bestPile) * 0.3, { pileId: bestPile.id }, "value", "setup");
      break;
    case "searchTwoFromPile":
      if (bestSearchPile && drawRoomAfterPlaying() >= 3) add((isUltraCpu() ? 620 : 360) + bestKnownPileScore(bestSearchPile, 3) * 0.25, { pileId: bestSearchPile.id }, "value", "setup");
      break;
    case "searchOneFromEachPile":
      if (drawRoomAfterPlaying() >= 1 && game.piles.some((pile) => pile.deck.length > 0)) add(420, {}, "value", "setup");
      break;
    case "drawPileDiscardTwo":
      if (bestPile && drawRoomAfterPlaying() >= 4) add(330 + bestKnownPileScore(bestPile, 6) * 0.12, { pileId: bestPile.id }, "value", "setup");
      break;
    case "takeDiscardToHandGainAction": {
      const best = bestDiscardCardIndex();
      if (best) add(220 + best.score, { discardIndex: best.index }, "value", "setup");
      break;
    }
    case "reviveUnit": {
    ...
    case "stealOpponentItems": {
      ...
      if (itemCount > 0 || isUltraCpu()) add((isUltraCpu() ? 540 : 250) + itemCount * 220, {}, "value", "setup");
      break;
    }
    case "swapUnits":
      if (enemyProtected) break;
      if (opponent.field.length > 0) {
        ...
        if (isUltraCpu()) {
          const ultraScore = 850 + (enemyValue - ownValue) * 0.65 + countSwing * 260 + bestSwing * 0.55;
          if (enemyValue > ownValue + 120 || opponent.field.length >= player.field.length || strongestEnemy) add(ultraScore, {}, "court-change");
        } else if (score > 420 && player.field.length <= opponent.field.length) add(120 + score, {}, "value");
      }
      break;
    default:
      break;
...
function chooseBestCpuSearchPile() {
  if (!isUltraCpu()) return chooseBestCpuPile();
  return [...game.piles]
    .filter((pile) => pile.deck.length > 0)
    .map((pile) => ({
      pile,
      score: bestKnownPileScore(pile, 3)
        + (pile.deck.includes("courtChange") ? 1800 : 0)
        + (pile.deck.includes("redCard") ? 520 : 0)
        + (pile.deck.includes("choiceScarf") ? 420 : 0),
    }))
    .sort((a, b) => b.score - a.score)[0]?.pile || chooseBestCpuPile();
}
...
function bestDiscardCardIndex() {
  return game.discard
    .map((cardId, index) => ({ index, cardId, score: hardCardIdScore(cardId) + (isUltraCpu() && cardId === "courtChange" ? 1200 : 0) }))
    .sort((a, b) => b.score - a.score)[0] || null;
}
...
function chooseHardCpuPileSearchIndexes() {
  ...
  if (pending.allPiles) {
    return game.piles.flatMap((pile) => pile.deck
      .map((cardId, index) => ({ value: `${pile.id}:${index}`, score: hardCardIdScore(cardId) + (isUltraCpu() && cardId === "courtChange" ? 1400 : 0) }))
      .sort((a, b) => b.score - a.score)
      .slice(0, 1)
      .map((entry) => entry.value));
  }
  ...
  if (!pile) return [];
  return pile.deck
    .map((cardId, index) => ({ index, score: hardCardIdScore(cardId) + (isUltraCpu() && cardId === "courtChange" ? 1400 : 0) }))
    .sort((a, b) => b.score - a.score)
...
function chooseCpuUltraSearchValue(choices = []) {
  return choices
    .map((choice) => ({ ...choice, score: hardCardIdScore(choice.cardId) + (choice.zone === "opponentHand" ? 35 : 0) + (isUltraCpu() && choice.cardId === "courtChange" ? 1500 : 0) }))
...
function cloneGameForCpu(source) {
  ...
  cloned.players?.[0]?.field?.forEach((unit) => {
    if (unit.item && !unit.item.revealed) {
      const publicUnit = cpuPublicUnitSnapshot(unit, 0);
      unit.hp = publicUnit.hp;
      unit.maxHp = publicUnit.maxHp;
      unit.power = publicUnit.power;
      unit.item = null;
    }
  });
  return cloned;
...
function immediateAttackValue(unit) {
  ...
  const bestKill = targets
    .map((target) => {
      const damage = estimateUnitDamageForCpu(cpuEffectivePower(unit, target, "attack", 1), 0);
      return effectiveDefenderHpForAttack(unit, target) <= damage ? 450 + unitThreat(target) : damage * 35;
    })
    .sort((a, b) => b - a)[0] || 0;
  const lifeDamage = canCpuAttackLifeNow(unit) ? estimateLifeDamageForCpu(cpuEffectivePower(unit, null, "lifeAttack", 1), 0) * 95 : 0;
...
function totalPossibleLifeDamage(extraPower = 0) {
  return game.players[1].field
    ...
    .reduce((sum, unit) => sum + estimateLifeDamageForCpu(cpuEffectivePower(unit, null, "lifeAttack", 1) + extraPower, 0), 0);
}
...
function keepComboBonus(cardId) {
  ...
  if (isUltraCpu() && cardId === "courtChange") return 1400;
  if (isUltraCpu() && ["doubleCheck", "excavation", "theSearch", "redCard", "choiceScarf"].includes(cardId)) return 520;
  if (isUltraCpu() && ["robbery", "wish"].includes(cardId)) return 420;
  return 0;
}
...
function chooseWeightedCpuChoice(choices) {
  ...
  if (isUltraCpu() && sorted[0].score >= 240) {
    const top = sorted.slice(0, 3);
    const close = top.filter((choice) => sorted[0].score - choice.score <= 120);
    return close[Math.floor(Math.random() * close.length)] || sorted[0];
  }
  const top = sorted.slice(0, 3);
...
function estimateLifeDamageForCpu(amount, targetPlayerId, sourceType = "attack") {
  let damage = Math.max(0, amount);
  const target = game.players[targetPlayerId];
  if (sourceType !== "attack" && target.mysticGuardUntilTurn > game.turn) return 0;
...
function estimateUnitDamageForCpu(amount, targetOwnerId, sourceType = "attack") {
  const target = game.players[targetOwnerId];
  if (sourceType !== "attack" && target.mysticGuardUntilTurn > game.turn) return 0;
  return target.damageReductionUntilTurn > game.turn ? Math.max(0, amount - 2) : Math.max(0, amount);
}

function hasCpuOwnQuagsireActive() {
  return game.players[1].field.some((unit) => CARD_DEFINITIONS[unit.cardId]?.effectKey === "ignorePowerIncreases");
}

function isPowerIncreaseItem(itemId) {
  return ["choiceBand", "lifePower", "lightBall", "contraryMask"].includes(itemId);
}

function cpuPublicUnitSnapshot(unit, ownerId = inferUnitOwnerId(unit)) {
  if (!unit) return unit;
  const snapshot = { ...unit, item: unit.item ? { ...unit.item } : null };
  if (ownerId !== 0 || !snapshot.item || snapshot.item.revealed) return snapshot;
  const itemCard = CARD_DEFINITIONS[snapshot.item.cardId];
  if (itemCard?.effectKey === "maxHpPlusTwo") {
    snapshot.maxHp = Math.max(snapshot.baseHp ?? 0, snapshot.maxHp - 2);
    snapshot.hp = Math.min(snapshot.hp, snapshot.maxHp);
  }
  if (itemCard?.effectKey === "pikachuPowerPlusSix" && snapshot.cardId === "pikachu") {
    snapshot.maxHp = Math.max(snapshot.baseHp ?? 0, snapshot.maxHp - 6);
    snapshot.hp = Math.min(snapshot.hp, snapshot.maxHp);
    if (snapshot.item.powerApplied) snapshot.power = Math.max(0, snapshot.power - 6);
  }
  if (itemCard?.effectKey === "attackPowerPlusTwo" && snapshot.item.powerApplied) {
    snapshot.power = Math.max(0, snapshot.power - 2);
  }
  snapshot.item = null;
  return snapshot;
}

function cpuVisibleHp(unit, ownerId = inferUnitOwnerId(unit)) {
  return Math.max(0, cpuPublicUnitSnapshot(unit, ownerId)?.hp ?? 0);
}

function cpuVisiblePower(unit, ownerId = inferUnitOwnerId(unit)) {
  return Math.max(0, cpuPublicUnitSnapshot(unit, ownerId)?.power ?? 0);
}

function cpuEffectivePower(unit, targetUnit, reason, ownerId = inferUnitOwnerId(unit)) {
  return engine.getEffectivePower(game, cpuPublicUnitSnapshot(unit, ownerId), targetUnit, reason, { silent: true });
}
...
function estimateOpponentLifeDamage(unit) {
  const raw = cpuEffectivePower(unit, null, "lifeAttack", 0);
...
function willLikelyDieNextTurn(unit) {
  const maxIncoming = game.players[0].field
    ...
    .map((enemy) => estimateUnitDamageForCpu(cpuEffectivePower(enemy, unit, "attack", 0), 1))
    .sort((a, b) => b - a)[0] || 0;
...
function hardCardScore(card) {
  ...
  if (card.type === "item") {
    ...
    return (values[card.id] || 170) + ultraCardBonus(card.id);
  }
  if (card.type === "action") {
    const values = {
      stoneThrow: 360,
    ...
    return (values[card.id] || 180) + ultraCardBonus(card.id);
  }
  return 0;
}

function ultraCardBonus(cardId) {
  if (!isUltraCpu()) return 0;
  const bonuses = {
    courtChange: 1600,
    redCard: 760,
    choiceScarf: 720,
...
function unitThreat(unit) {
  ...
  const knownItem = cpuKnownItemCardId(unit, ownerId);
  const visibleHp = cpuVisibleHp(unit, ownerId);
  const visiblePower = cpuVisiblePower(unit, ownerId);
  const displayedPower = knownItem === "lifePower" ? Math.max(visiblePower, visibleHp) : visiblePower;
  return Math.max(0, visibleHp) * 45 + Math.max(0, displayedPower) * 80 + hardUnitCardScore(card) * 0.45 + (unit.item ? 80 : 0);
}
...
function effectiveDefenderHpForAttack(attacker, target) {
  ...
  if (["useTargetPowerAsHp", "useTargetPowerAsHpNoSummonSick"].includes(attackerEffect)) {
    return Math.max(0, cpuEffectivePower(target, attacker, "status", inferUnitOwnerId(target)));
  }
  return cpuVisibleHp(target, inferUnitOwnerId(target));
}
...
function chooseHardCpuUnitAbility() {
  ...
  player.field.filter((unit) => unit.canAct).forEach((unit) => {
    ...
    if (card.effectKey === "attackOrGainLife" && player.life <= 9 && !canCpuLethalByAttacksNow()) {
      const kyogrePlan = isUltraCpu() && player.life < 10 && (player.field.some((ally) => ally.cardId === "kyogre") || player.hand.includes("kyogre"));
      const healScore = isCpuInLethalDanger() ? 540 + (12 - player.life) * 70 : 120 + (9 - player.life) * 35 + (kyogrePlan ? 380 : 0);
      if (player.life <= 7 || isCpuInLethalDanger() || kyogrePlan) {
    ...
    if (card.effectKey === "gainSpecialInsteadAttack" && !canCpuLethalByAttacksNow()) {
      const hasLightBall = cpuKnownItemCardId(unit, 1) === "lightBall";
      if (!hasLightBall && !isCpuInLethalDanger()) {
        choices.push({
          unitId: unit.id,
          score: 360 + dangerScore(unit) * 0.12,
          label: "CPU スペシャル入手",
          payload: { ability: "gainSpecial", unitId: unit.id },
          fx: "fx-item",
        });
      }
    }
    if (card.effectKey === "sleepTargetNextTurn") {
      opponent.field.forEach((target) => {
        if (isUltraCpu() && canCpuKillTargetWithReadyAttackers(target)) return;
        if (target.canAct || dangerScore(target) >= 420) {
          choices.push({ unitId: unit.id, targetId: target.id, score: 230 + dangerScore(target) * (isUltraCpu() ? 0.55 : 0.35), label: "CPU 召喚酔い", payload: { ability: "sleepTargetNextTurn", unitId: unit.id, targetUnitId: target.id }, fx: "fx-item" });
        }
      });
    }
    if (card.effectKey === "damageOnSummonZeroPowerAndReturn") {
      opponent.field.forEach((target) => {
        if (isUltraCpu() && (target.power <= 2 || canCpuKillTargetWithReadyAttackers(target))) return;
        if (target.power >= 2) {
          choices.push({
            unitId: unit.id,
            targetId: target.id,
...
function scoreDoublePowerAbility(unit) {
  if (hasCpuOwnQuagsireActive()) return -500;
  if (unit.power <= 1) return -500;
  ...
  if (!canLifeLethal && !canKillImportantTarget && room < 3) return -500;
  if (!canLifeLethal && room < 1) return -500;
...
function chooseHardCpuAttack() {
  ...
  player.field.filter((unit) => unit.canAct).forEach((attacker) => {
    const lifeDamage = estimateLifeDamageForCpu(cpuEffectivePower(attacker, null, "lifeAttack", 1), 0);
    ...
    filterAttackTargets(opponent.field, attacker).forEach((target) => {
      const damage = estimateUnitDamageForCpu(cpuEffectivePower(attacker, target, "attack", 1), 0);
      const counter = player.noCounterThisTurn ? 0 : estimateUnitDamageForCpu(cpuEffectivePower(target, attacker, "counter", 0), 1);
      ...
      const multiAttackKills = CARD_DEFINITIONS[attacker.cardId]?.effectKey === "attackAllEnemies" || cpuKnownItemCardId(attacker, 1) === "boomerang"
        ? opponent.field.filter((unit) => effectiveDefenderHpForAttack(attacker, unit) <= damage).length
        : 0;
      ...
      if (isUltraCpu() && !kills && multiAttackKills === 0 && (!combinedPlan.canKill || !combinedPlan.attackers.includes(attacker.id))) return;
      if (!kills && combinedPlan.canKill && combinedPlan.attackers.includes(attacker.id)) {
        score += 520 + dangerScore(target) * 0.55 + damage * 55;
      }
      if (!kills && !combinedPlan.canKill) {
        score -= 1600;
      }
      ...
      if (CARD_DEFINITIONS[attacker.cardId]?.effectKey === "attackAllEnemies" || cpuKnownItemCardId(attacker, 1) === "boomerang") {
        score += opponent.field.length * 80 + multiAttackKills * 330;
      }
      choices.push({
        attackerId: attacker.id,
...
function chooseCpuLethalLifeAttack() {
  ...
  const lifeChoices = game.players[1].field
    ...
    .map((attacker) => ({
      attackerId: attacker.id,
      damage: estimateLifeDamageForCpu(cpuEffectivePower(attacker, null, "lifeAttack", 1), 0),
      score: 30000,
      label: "CPU ライフ攻撃",
      reason: "lethal",
    }))
...
function isSuicideIntoUnkillableWall(choice) {
  ...
  const damage = estimateUnitDamageForCpu(cpuEffectivePower(attacker, defender, "attack", 1), 0);
  const counter = game.players[1].noCounterThisTurn ? 0 : estimateUnitDamageForCpu(cpuEffectivePower(defender, attacker, "counter", 0), 1);
...
function dangerousTargetPlan(target) {
  const attackers = game.players[1].field.filter((unit) => unit.canAct)
    .map((unit) => ({
      id: unit.id,
      damage: estimateUnitDamageForCpu(cpuEffectivePower(unit, target, "attack", 1), 0),
      risk: estimateUnitDamageForCpu(cpuEffectivePower(target, unit, "counter", 0), 1) >= unit.hp ? unitThreat(unit) * 0.25 : 0,
    }))
...
function dangerScore(unit) {
  ...
  const ownerId = inferUnitOwnerId(unit);
  const knownItem = cpuKnownItemCardId(unit, ownerId);
  const publicPower = cpuVisiblePower(unit, ownerId);
  const publicHp = cpuVisibleHp(unit, ownerId);
  const visiblePower = knownItem === "lifePower" ? Math.max(publicPower, publicHp) : publicPower;
